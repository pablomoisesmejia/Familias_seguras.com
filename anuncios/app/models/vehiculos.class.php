...
class Vehiculos extends Validator
{
    ...
    {
        $sql = 'SELECT v.PK_id_vehiculo, v.FK_id_usuario, mv.modelos_vehiculo, mar.marca_vehiculo, v.anio, v.color, v.kilometraje, v.transmision, v.motor, v.vidrios_electricos, v.espejos_electricos, aire_acondicionado, bolsas_aire, v.sistema_eco, v.mandos_timon, v.rines_especiales, v.camara_trasera, v.sensores_parqueo, v.bluetooth, v.combustible, v.sunroof, v.luces_xenon, v.cruise_control, v.mando_distancia, v.gps, v.tapiceria_cuero, v.dvd_trasero, v.valor, ua.whatsapp, ua.telefono
        FROM vehiculos v INNER JOIN modelos_vehiculos mv ON v.FK_id_modelo = mv.PK_id_modelo_vehiculo 
        INNER JOIN marcas_vehiculos mar ON mv.FK_id_marca_vehiculo = mar.PK_id_marca_vehiculo
        INNER JOIN usuarios_anuncios ua ON v.FK_id_usuario = ua.id_usuario
    ...
    {
        $sql = 'SELECT v.PK_id_vehiculo, v.FK_id_usuario, mv.modelos_vehiculo, mar.marca_vehiculo, v.anio, v.valor FROM vehiculos v INNER JOIN modelos_vehiculos mv ON v.FK_id_modelo = mv.PK_id_modelo_vehiculo INNER JOIN marcas_vehiculos mar ON mv.FK_id_marca_vehiculo = mar.PK_id_marca_vehiculo';
        $params = array(null);
        return Database::getRows($sql, $params);
    }
